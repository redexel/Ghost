...
from datetime import datetime # read time
...
class RunConfiguration:
    ...
    def __init__(self):
        ...
        self.IFACE_TO_TAKE_DOWN = ''  # Interface that ghost puts into monitor mode
    ...
    def ConfirmRunningAsRoot(self):
        if os.getuid() != 0:
            print R + ' [!]' + O + ' ERROR:' + G + ' ghost' + O + ' must be run as ' + R + 'root' + W
            print R + ' [!]' + O + ' login as root (' + W + 'su root' + O + ') or try ' + W + 'sudo ./ghost.py' + W
            exit(1)

    def ConfirmCorrectPlatform(self):
        if not os.uname()[0].startswith("Linux") and not 'Darwin' in os.uname()[0]:  # OSX support, 'cause why not?
            print O + ' [!]' + R + ' WARNING:' + G + ' ghost' + W + ' must be run on ' + O + 'linux' + W
            exit(1)

    def CreateTempFolder(self):
        from tempfile import mkdtemp

        self.temp = mkdtemp(prefix='ghost')
    ...
    def exit_gracefully(self, code=0):
        ...
        mac_change_back()
        print GR + " [+]" + W + " quitting"  # ghost will now exit"
        print ''
        # GTFO
        exit(code)
    ...
    def build_opt_parser(self):
        ...
        global_group.add_argument('--update', help='Check and update ghost.', default=False, action='store_true',
                                  dest='update')
    ...
    def upgrade(self):
        ...
        try:
            ...
            if revision == -1:
                ...
                try:
                    sock = urllib.urlopen('https://github.com/derv82/ghost/raw/master/ghost.py')
                    page = sock.read()
                except IOError:
                    page = ''
                if page == '':
                ...
                f = open('update_ghost.sh', 'w')
                f.write('''#!/bin/sh\n
                           rm -rf ''' + this_file + '''\n
                           mv ghost_new.py ''' + this_file + '''\n
                           rm -rf update_ghost.sh\n
                           chmod +x ''' + this_file + '''\n
                          ''')
                f.close()

                # Change permissions on the script
                returncode = call(['chmod', '+x', 'update_ghost.sh'])
                if returncode != 0:
                    print R + ' [!]' + O + ' permission change returned unexpected code: ' + str(returncode) + W
                    self.exit_gracefully(1)
                # Run the script
                returncode = call(['sh', 'update_ghost.sh'])
                if returncode != 0:
                ...
                print GR + ' [-]' + W + ' your copy of ghost is ' + G + 'up to date' + W

        except KeyboardInterrupt:
            print R + '\n (^C)' + O + ' ghost upgrade interrupted' + W
        self.exit_gracefully(0)
...
class RunEngine:
    ...
    def scan(self, channel=0, iface='', tried_rtl8187_fix=False):
        ...
        """
        remove_airodump_files(self.RUN_CONFIG.temp + 'ghost')

        command = ['airodump-ng',
                   '-a',  # only show associated clients
                   '--write-interval', '1', # Write every second
                   '-w', self.RUN_CONFIG.temp + 'ghost']  # output file
        if channel != 0:
            command.append('-c')
        ...
        try:
            ...
            while True:
                time.sleep(0.3)
                if not os.path.exists(self.RUN_CONFIG.temp + 'ghost-01.csv') and time.time() - time_started > 1.0:
                    print R + '\n [!] ERROR!' + W
                    ...
                    print R + ' [!]' + O + ' ghost is unable to generate airodump-ng output files' + W
                    print R + ' [!]' + O + ' you may want to disconnect/reconnect your wifi device' + W
                    self.RUN_CONFIG.exit_gracefully(1)

                (targets, clients) = self.parse_csv(self.RUN_CONFIG.temp + 'ghost-01.csv')
                ...
                if self.RUN_CONFIG.VERBOSE_APS and len(targets) > 0:
                    ...
                    if not self.RUN_CONFIG.WPS_DISABLE:
                        wps_check_targets(targets, self.RUN_CONFIG.temp + 'ghost-01.cap', verbose=False)
        ...
        if not self.RUN_CONFIG.WPS_DISABLE:
            wps_check_targets(targets, self.RUN_CONFIG.temp + 'ghost-01.cap')
...
def banner(RUN_CONFIG):
    ...
    print "  ___ _           _         _     __    __ _  __ _     ___               _             "
    print " / _ \ |__   ___ | |__  ___| |_  / / /\ \ (_)/ _(_)   / __\ __ __ _  ___| | _____ _ __ "
    print "/ /_\/ '_ \ / _ \| '_ \/ __| __| \ \/  \/ / | |_| |  / / | '__/ _` |/ __| |/ / _ \ '__|"
    print "/ /_\\| | | | (_) | | | \__ \ |_   \  /\  /| |  _| | / /__| | | (_| | (__|   <  __/ |   "
    print "\____/|_| |_|\___/|_| |_|___/\__|   \/  \/ |_|_| |_| \____/_|  \__,_|\___|_|\_\___|_|   "


    print "###################### Ghost v.1 - Automated wireless auditor ###########################"
...
def help():
    ...
    print head + '\n   EXAMPLE' + W
    print sw + '\t./ghost.py ' + W + '-wps -wep -c 6 -pps 600' + W
    print ''
...
class WPAAttack(Attack):
    ...
    def wpa_get_handshake(self):
        ...
        try:
            ...
            log=open("logmess.txt","a")
            if (os.path.getsize("logmess.txt")>0):
                log.write("\n" + mess + " " +str(datetime.now())+"\n" )
            else:

                log.write(mess + " " +str(datetime.now())+"\n")
            log.close()
            print(mess)
...
class WPSAttack(Attack):
    ...
    def attack_wps(self):
        ...
        cmd = ['reaver',
               ...
               # TODO: Save session to a file, maybe ~/.ghost/
               '--session', '/dev/null',
               '-c', self.target.channel,
               '-vv']  # verbose output
